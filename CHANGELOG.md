...
- **Freshness lifetime directive precedence** — the cache now follows RFC 9111 §4.2.1 and uses the first applicable freshness directive (`s-maxage`, then `max-age`, then `Expires`, then the default heuristic). Previously Ferron took the minimum of all candidates, which could produce shorter TTLs than the origin intended when directives disagreed.
- **304 revalidation header merge** — header field values received in a `304 Not Modified` response now replace the stored values for the same field names, per RFC 9111 §4.3.4. Previously Ferron appended them, so repeated revalidations accumulated duplicate `Cache-Control` (and other) values on the stored entry.
- **No heuristic TTL for `must-revalidate`** — per RFC 9111 §4.2.4, Ferron no longer generates a default freshness lifetime when the response carries `must-revalidate`, `proxy-revalidate`, or `s-maxage` without an explicit `max-age`, `s-maxage`, or `Expires`. Such responses are not stored. Explicit `max-age=0` or `s-maxage=0` is now stored with a zero TTL (always revalidates) instead of being rejected.
- **Authorized responses with `must-revalidate` are storable** — per RFC 9111 §3.5, Ferron now stores responses to authorized requests when the response includes `must-revalidate` or `proxy-revalidate`, in addition to the previously supported `public` and `s-maxage`. Previously only `public` and `s-maxage` permitted storage, so such responses were rejected as `authorization-public`.
- **`no-cache="field-names"` support** — per RFC 9111 §5.2.2.3, Ferron now stores responses whose `Cache-Control` carries a `no-cache` directive with field names (for example `no-cache="Set-Cookie"`), and removes the named fields from the stored entry. Previously any `no-cache` form caused the response to be rejected as not cacheable.
- **`206 Partial Content` cacheable by default** — Ferron now treats `206` responses as cacheable by default, matching the RFC 9111 §3.2 status list. Requests that carry a `Range` header still bypass the cache entirely.
- **Request `no-store` no longer bypasses lookup** — per RFC 9111 §5.2.1.5, a `Cache-Control: no-store` request directive forbids storing the request and its response, but does not forbid serving a stored response. Ferron now serves a fresh stored entry to a `no-store` request instead of bypassing the cache entirely. The response to such a request is still never stored.
- **PURGE authorization scoped to host** — a `PURGE` request is now authorized by basic-auth credentials only when a `basic_auth` block is in scope for the request host. Previously any authenticated user (including credentials belonging to another host's `basic_auth`) could purge a host's cache. IP allow-listing (`purge_allowed_ips`) is unchanged.
- **PURGE scoped to the requesting host** — in a shared cache zone, a `PURGE` (or `X-LiteSpeed-Purge`) from one host now only invalidates entries cached for that same host. Previously a host-wide purge in a named or global zone wiped every tenant's entries, including other hosts' tags and URLs.
- **Propagated purges now authenticate** — a `PURGE` tagged with `X-Purge-Source: propagation` must carry an `X-Purge-Secret` matching the configured `purge_propagation.shared_secret` (constant-time comparison). Otherwise Ferron rejects it with 403. Previously the propagation tag was trusted on its own, so any client could claim it to bypass the normal purge authorization.
- **Cache keys use the resolved vhost** — the cache key derives from the vhost Ferron matched for the request instead of the raw client-supplied `Host` header, and is normalized to lowercase. Previously a `Host` header differing in case, with a trailing dot, or spoofing another vhost produced a distinct key, fragmenting the cache and letting a client miss other tenants' entries.
- **Private responses are never keyed on the client IP** — a private-scoped response is stored only when the request carries client identity: an authenticated user, a recognized private cookie, or a cookie named in `vary_cookies`. Otherwise Ferron serves it uncached. Previously Ferron keyed private entries on the client IP address alone, so clients behind the same CGNAT address could receive each other's private responses.
- **Private cache key cardinality bound** — the private cache key no longer falls back to every request cookie. It uses at most 8 cookie components and truncates each cookie value to 256 characters. The per-URL variant map is capped at 64 entries and evicts the oldest variant. This prevents arbitrary cookies from exploding the cache key space and the variant map.
- **Hop-by-hop headers stripped from stored and served responses** — per RFC 9110 §7.6.1, Ferron removes the hop-by-hop header list (`Connection`, `Keep-Alive`, `Proxy-Authenticate`, `Proxy-Authorization`, `TE`, `Trailer`, `Transfer-Encoding`, `Upgrade`) plus any field named in the `Connection` header, both when storing an entry and when serving it from the cache. `Age` is also dropped on store. Previously these headers could leak from the origin through the cache.
- **Origin `Set-Cookie` never stored verbatim** — Ferron strips `Set-Cookie` from every stored response, public or private, instead of keeping it on private entries and replaying it on every hit. Stale session credentials no longer leak from the cache, and a `304` revalidation cannot merge a `Set-Cookie` into a stored entry. `LSC-Cookie` metadata is stored separately and is still rehydrated into `Set-Cookie` on a cache hit.
- **`Cache-Status` labels `stale-if-error` correctly** — a stale response served because revalidation failed with a 5xx now reports `detail=stale-if-error` in the `Cache-Status` header instead of the previous `detail=stale-while-revalidate`. The two stale-serving mechanisms are now distinguishable in logs and client-facing cache status.
- **SWR leader receives the fresh response** — during `stale-while-revalidate`, the request that revalidates the entry with the upstream now receives the fresh response it just stored, labeled `detail=revalidated` in `Cache-Status`. Previously the leader was served the same stale entry as concurrent followers, labeled as a stale hit, so the revalidating client waited for the upstream yet still got stale content.
- **Singleflight keys on the full entry key** — the in-flight request deduplication mechanism now coalesces concurrent fetches on the resolved entry key (scope plus `Vary` values plus private key) instead of the base URL. Two requests that map to different variants of the same URL no longer share a single upstream fetch, so a follower cannot be woken to a fresh entry that does not match its own variant.
- **Coalesced fetch wait has a timeout** — a request that coalesces onto an in-flight upstream fetch now waits up to `coalesce_timeout` (default 5 seconds) for the leader. On timeout it stops coalescing and fetches from the upstream itself, so one hung upstream request cannot stall concurrent requests for the same entry indefinitely.
- **Cache request metrics are no longer double-counted** — a request that revalidates an entry with the upstream previously counted as a `hit` at lookup time and then again as `revalidated` or `miss` after the upstream response arrived. Each request now contributes exactly one `ferron.cache.requests` increment, labeled with the final outcome. The SWR leader no longer emits an extra eviction count before it revalidates; the store path records evictions, stores, and the `revalidated` request once.
- **Orphaned URL variant registries are cleaned up** — when the last cached entry for a URL expires past its `stale-while-revalidate` window or is evicted for size, Ferron now removes the URL's variant registry instead of leaking it in memory. A later request for that URL is a plain miss. The registry survives as long as at least one entry of the URL remains.
- **Expired-entry sweeps are throttled** — Ferron no longer scans the whole cache for expired entries on every lookup and insert. The scan runs at most once per second. Expired entries are still never served in between, because lookups skip anything past its lifetime.
- **LiteSpeed headers no longer decide scope or TTL without the override** — `X-LiteSpeed-Cache-Control` previously influenced the cache scope and freshness lifetime even when `litespeed_override_cache_control` was off. It now only rejects storage through `no-store` or `no-cache`; standard HTTP caching rules decide scope and TTL.
- **Fresh hits answer client conditionals with a local 304** — when a cached entry is still fresh and the client sends a matching `If-None-Match` (or `If-Modified-Since` for `GET`/`HEAD`), Ferron now responds `304 Not Modified` from the cache with the stored validators, without contacting the origin. `If-None-Match` takes precedence over `If-Modified-Since`, `*` matches any stored entity tag, and entity tags are compared weakly per RFC 9110 §8.8.3.2. A `Range` request is still never answered from the cache.
- **Conditional request headers no longer fragment cache variants** — Ferron no longer treats the conditional and range request headers (`If-Match`, `If-Modified-Since`, `If-None-Match`, `If-Range`, `If-Unmodified-Since`, `Range`) listed in an origin's `Vary` header as cache-variant dimensions. Previously a `Vary: If-None-Match` from the origin meant every conditional request missed the cache and went upstream, because its entity-tag value differed from the stored variant's. These headers select validators at serve time, so the cache now stores a single variant and evaluates conditionals against it.
- **Cache key fingerprint hides the query string** — the `ferron.cache.key_fingerprint` access-log field now drops the URL query string before truncating the key. Previously a truncated key preserved up to 48 characters of the query, so URL parameters such as access tokens could leak into the logs. The scope and `Vary` components after the base URL remain, so the fingerprint still distinguishes variants.
- **Cache-key vary values are whitespace-normalized and sorted** — vary header and cookie values embedded in a cache key are now trimmed, their internal whitespace runs collapsed, and repeated header values sorted, so requests that differ only in formatting (for example `Accept-Language: en, fr` vs `en  fr` with the lines in a different order) share the same key instead of fragmenting into separate entries.
