...
if [[ \$link -eq 1 ]]; then
  exec clang --sysroot=${sysroot} --target=${llvm_target} -fuse-ld=lld \$filtered_args
else
  exec clang --sysroot=${sysroot} --target=${llvm_target} \$filtered_args
fi
WRAPPER
	cat > "${cxx_wrapper}" <<WRAPPER
#!/bin/bash
...
WRAPPER
	...
	cat > "${linker_wrapper}" <<WRAPPER
#!/bin/bash
exec clang --target=${llvm_target} --sysroot="${sysroot}" -fuse-ld=lld "\$@"
WRAPPER
	chmod +x "${linker_wrapper}"
