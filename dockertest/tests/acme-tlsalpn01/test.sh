...
if [ "$TEST_EXIT_CODE" -eq 0 ]; then
    echo "Automatic TLS connection (TLS-ALPN-01 ACME challenge) test passed!"
else
    echo "Automatic TLS connection (TLS-ALPN-01 ACME challenge) test failed!" >&2
...
TEST_EXIT_CODE=$?
if [ "$TEST_EXIT_CODE" -eq 0 ]; then
    echo "On-demand automatic TLS (TLS-ALPN-01 ACME challenge) connection test passed!"
else
    echo "On-demand automatic TLS (TLS-ALPN-01 ACME challenge) connection test failed!" >&2
    echo "  Exit code: $TEST_EXIT_CODE" >&2
    TEST_FAILED=1
fi
