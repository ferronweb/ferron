...
rm -rf certs
mkdir -p certs
docker run --rm -v ./certs:/etc/certs alpine/openssl req -x509 -newkey rsa:4096 -keyout /etc/certs/server.key -out /etc/certs/server.crt -days 365 -nodes -subj "/CN=localhost" > /dev/null 2>&1 || (echo "Failed to generate TLS key pair for a test suite" >&2)
