...
| ---------------------------------------- | ------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ferron.cache.requests`                  | Counter | `ferron.cache.zone`, `ferron.cache.result`, `ferron.cache.scope`, `ferron.cache.reason` (miss/bypass only) | Cache hits, misses, bypasses, and revalidations. Each request contributes exactly one counter increment, labeled with the final outcome (`hit`, `stale`, `revalidated`, `miss`, or `bypass`). For `miss`/`bypass` outcomes, `ferron.cache.reason` additionally carries the bounded policy reason (for example `not-cacheable`, `zero-ttl`, `private-no-identity`, `request-only-if-cached`), so a dashboard can break down *why* misses/bypasses happened without grepping access logs. |
...
| `ferron.cache.persistence_errors`        | Counter | `ferron.cache.zone`                                                    | Cache persistence failures for the zone: a journal flush failure (which also disables on-disk persistence for that zone until restart) or a snapshot compaction failure (which is retried and does not disable the zone). |
| `ferron.cache.persistence_dropped_records` | Counter | `ferron.cache.zone`                                                  | Journal records dropped because the zone's write queue exceeded capacity under backpressure. The newest record for every key always survives.                                                |
| `ferron.cache.persistence_active`        | Gauge   | `ferron.cache.zone`                                                    | `0` once a journal flush failure has disabled on-disk persistence for the zone (the zone keeps serving from memory); not emitted while the zone is healthy.                                   |
...
| `ferron.cache.key_fingerprint`           | string | Truncated cache key (up to 48 characters) with the query string removed, useful for diagnosing why a specific request missed. The query is stripped so URL parameters (for example access tokens) never reach the logs. If a query string was present, a short non-reversible tag (`q=<16 hex chars>`, an xxh3 hash) is appended so two requests differing only by query no longer collapse into an identical fingerprint. If the base itself had to be truncated, a second tag (`h=<16 hex chars>`) of the untruncated base is appended for the same reason. |
| `ferron.cache.detail`                    | string | Why the response was not served from, or not stored in, the cache (for example `response-no-store`, `not-cacheable`, `authorization-public`, `public-set-cookie`, `zero-ttl`, `private-no-identity`, `response-too-large`, `unsupported-litespeed-vary-value`), when applicable. Present on `miss` and `bypass` outcomes, mirroring the `ferron.cache.detail` span attribute so the reason survives after the request without needing a live trace capture. |
