...
Ferron applies rewrites early in the request pipeline, before proxying or static file serving, so routing uses the rewritten URL. The client sees no redirect — the rewrite is transparent.
