...
async fn test_acme_common(challenge_type: &str, hostname: &str, extra_host_config: &str) {
  ...
  std::fs::write(cert_dir.path().join("ca.key"), cert.signing_key.serialize_pem()).unwrap();
...
"#,
  ...
  let port = ferron.get_host_port_ipv4(ContainerPort::Tcp(443)).await.unwrap();
  ...
  for _ in 0..60 {
    ...
    if let Ok(response) = client.get(format!("https://{}:{}/", hostname, port)).send().await {
...
async fn test_acme_broken_cache() {
  ...
  test_acme_common("http-01", "ferron-brokencache", "auto_tls_cache \"/root/cache\"").await;
}

#[tokio::test]
async fn test_acme_ondemand() {
  test_acme_common("tls-alpn-01", "ferron-ondemand", "auto_tls_on_demand").await;
}

#[tokio::test]
async fn test_acme_http01_ondemand() {
  test_acme_common("http-01", "ferron-http01-ondemand", "auto_tls_on_demand").await;
}
