...
/// Tests that when the primary ACME provider fails, the fallback provider is used.
/// The primary provider points to a non-existent directory, so account creation
/// will fail. The fallback provider points to Pebble, which should succeed.
#[tokio::test]
async fn test_acme_fallback_provider() {
    let _ = rustls::crypto::ring::default_provider().install_default();
  ...
  "pebble": {
    ...
    // 3. Write Ferron config with fallback providers
    // The primary provider points to a non-existent host (will fail).
    // The fallback provider points to Pebble (will succeed).
    ferron_config
        .as_file_mut()
        .write_all(
            format!(
...
"#
    ...
    // Poll until success — the fallback provider should be used after the primary fails
    let mut success = false;
    for _ in 0..100 {
        if let Ok(response) = client
            .get(format!("https://ferron-fallback:{}/", port))
            .send()
            .await
            && response.status().is_success()
        {
