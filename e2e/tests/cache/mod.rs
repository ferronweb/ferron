...
/// F28: a fresh cache hit with client conditionals must be answered locally
/// with 304 Not Modified instead of replaying the full 200 body.
///
/// The stored representation carries an ETag; a client sending
/// `If-None-Match` (or `If-Modified-Since`) that still matches gets a local
/// 304 with an empty body and a "hit" Cache-Status. A non-matching tag gets
/// the full 200 body.
#[tokio::test]
async fn test_fresh_hit_serves_local_304_for_client_conditionals() {
    let _ = rustls::crypto::ring::default_provider().install_default();
  ...
  "#
    ...
    common::write_file(webroot_dir.path().join("cond.txt"), b"cond-v1").unwrap();
    ...
    let etag = resp1
        ...
        .unwrap()
        .to_string();
    assert_eq!(resp1.text().await.unwrap(), "cond-v1");

    // Request 2: fresh hit + matching If-None-Match -> local 304, no body.
    let resp2 = client
        .get(format!("{base_url}cond.txt"))
        .header("If-None-Match", &etag)
        .send()
    ...
    assert_eq!(
        ...
        "fresh hit with matching If-None-Match must yield 304"
    );
    assert!(
        String::from_utf8_lossy(resp2.headers().get("Cache-Status").unwrap().as_bytes())
            .contains("hit"),
        "local 304 must be a cache hit"
    );
    assert_eq!(
        resp2.headers().get("ETag").map(|v| v.to_str().unwrap()),
        Some(etag.as_str()),
        "304 must keep the stored ETag"
    );
    assert!(
        resp2.text().await.unwrap().is_empty(),
    ...
    // Request 3: fresh hit + non-matching If-None-Match -> full 200 body.
    let resp3 = client
        .get(format!("{base_url}cond.txt"))
        .header("If-None-Match", "\"does-not-match\"")
        .send()
        .await
        .unwrap();
    assert_eq!(resp3.status(), reqwest::StatusCode::OK);
    assert_eq!(resp3.text().await.unwrap(), "cond-v1");

    // Request 4: weak comparison matches even when the client sends the
    // strong form of the same opaque tag (If-None-Match uses weak comparison
    // for GET/HEAD per RFC 9110 §8.8.3.2).
    let strong_etag = etag.strip_prefix("W/").unwrap_or(&etag);
    let resp4 = client
        .get(format!("{base_url}cond.txt"))
        .header("If-None-Match", strong_etag)
        .send()
        .await
        .unwrap();
    assert_eq!(
        resp4.status(),
        reqwest::StatusCode::NOT_MODIFIED,
        "weak If-None-Match must still match"
    );

    // Request 5: If-Modified-Since newer than the stored Last-Modified.
    let resp5 = client
        .get(format!("{base_url}cond.txt"))
    ...
    assert_eq!(
        resp5.status(),
        reqwest::StatusCode::NOT_MODIFIED,
        "future If-Modified-Since must yield 304"
    );
