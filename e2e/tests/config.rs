...
async fn test_config() {
  ...
  config_file
    ...
    .write_all(
      ...
      ferron:80 {
        ...
        // Deny some scanner scan
        if "SOMESCANNER_SCAN" {
          status 403
        }

        // Redirect non-existent URLs to the basic.txt file
        error_config 404 {
          status 302 regex="^/(?!basic\\.txt(?:$|[?#]))" location="/basic.txt"
        }
      }
  "#
      .as_bytes(),
  ...
  self::common::write_file(webroot_dir.path().join("basic.txt"), basic_content.as_bytes()).unwrap();
  ...
  // reqwest follows redirects by default, so we check content
  assert_eq!(response.status(), reqwest::StatusCode::OK);
  assert_eq!(response.text().await.unwrap(), basic_content);
