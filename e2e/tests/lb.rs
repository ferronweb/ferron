...
async fn test_lb() {
  ...
  // Start backends
  let _backend1 = create_backend_container(network, "backend-1").await.unwrap();
  let _backend2 = create_backend_container(network, "backend-2").await.unwrap();
  let _backend3 = create_backend_container(network, "backend-3").await.unwrap();
  ...
  config_file
    ...
    .write_all(
      br#"
ferron-random:80 {
  proxy "http://backend-1:3000"
  proxy "http://backend-2:3000"
  proxy "http://backend-3:3000"
