...
"#,
    ...
    let _ocsp = create_ocsp_container(&network, cert_dir.path(), false)
        .await
        .unwrap();
...
"#,
    ...
    // Start OCSP responder container
    let ocsp = create_ocsp_container(&network, cert_dir.path(), false)
        .await
        .unwrap();
...
"#,
    ...
    // Start OCSP responder container
    let _ocsp = create_ocsp_container(&network, cert_dir.path(), true)
        .await
        .unwrap();
