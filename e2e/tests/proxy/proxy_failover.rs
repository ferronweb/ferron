...
/// Test that HTTP 503 from upstream is returned directly.
///
/// Inspired by nginx-tests proxy_next_upstream.t — Ferron retries on
/// connection-level failures only (not HTTP error responses). When the
/// backend returns 503, Ferron returns 503 to the client without retrying.
#[tokio::test]
async fn test_failover_on_http_503() {
    let _ = rustls::crypto::ring::default_provider().install_default();
    ...
    let network = "e2e-test-failover-503";

    // backend-flaky returns 503 for first request, then succeeds
    let _backend_flaky = create_backend_container(network, "backend-flaky", "backend-flaky", 1)
        .await
        .unwrap();
...
"#,
    ...
    // First request to /unstable will get 503 from backend-flaky.
    // Ferron does NOT retry on HTTP 503 — only on connection-level failures.
    let response = client
    ...
    assert_eq!(response.text().await.unwrap(), "unstable:backend-flaky");
