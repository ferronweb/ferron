...
impl RProxyTestContext {
  async fn new(test_name: &str) -> Self {
    ...
    // Start backend
    let backend = create_backend_container(&network, cert_dir.path()).await.unwrap();
...
:80 {
  ...
  location "/header" {
    proxy_request_header_replace "X-Some-Header" "something"
  }
...
async fn test_x_forwarded_for() {
  ...
  let response = ctx.client.get(format!("{}/ip", ctx.base_url)).send().await.unwrap();
  assert_eq!(response.status(), reqwest::StatusCode::OK);
...
async fn test_custom_header() {
  ...
  let response = ctx.client.get(format!("{}/header", ctx.base_url)).send().await.unwrap();
  assert_eq!(response.status(), reqwest::StatusCode::OK);
...
async fn test_bad_gateway() {
  ...
  let response = ctx.client.get(format!("{}/unsafe", ctx.base_url)).send().await.unwrap();
...
async fn test_tls_backend() {
  ...
  let response = ctx.client.get(format!("{}/tls", ctx.base_url)).send().await.unwrap();
  assert_eq!(response.status(), reqwest::StatusCode::OK);
  assert_eq!(response.text().await.unwrap(), "Hello, World!");
}
