...
impl StaticTestContext {
  ...
  "#
    ...
    common::write_file(webroot_dir.path().join("basic.txt"), basic_content.as_bytes()).unwrap();
    common::write_file(webroot_dir.path().join("unicode.txt"), unicode_content.as_bytes()).unwrap();
...
async fn test_etags() {
  ...
  let response = ctx
    ...
    .await
    .unwrap();
  let etag_gzip = response.headers().get(header::ETAG).unwrap().to_str().unwrap();
  let response = ctx
    .client
    .get(&format!("{}/basic.txt", ctx.base_url))
    .header(header::ACCEPT_ENCODING, "gzip")
