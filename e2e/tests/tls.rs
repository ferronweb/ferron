...
async fn create_ferron_container(
  ...
  ferron_image
    ...
    .with_mount(Mount::bind_mount(cert_file.to_string_lossy(), "/etc/cert.pem"))
    .with_mount(Mount::bind_mount(key_file.to_string_lossy(), "/etc/key.pem"))
    .start()
    .await
}
...
async fn test_tls_http_1() {
  ...
  cert_file.as_file_mut().write_all(cert.cert.pem().as_bytes()).unwrap();
  key_file
    .as_file_mut()
...
"#
  ...
  let port = container.get_host_port_ipv4(ContainerPort::Tcp(443)).await.unwrap();
  ...
  let response = client.get(format!("https://localhost:{}/", port)).send().await.unwrap();
...
"#
  ...
  let port = container.get_host_port_ipv4(ContainerPort::Udp(443)).await.unwrap(); // QUIC uses UDP
  ...
  let mut client_endpoint = h3_quinn::quinn::Endpoint::client("0.0.0.0:0".parse().unwrap()).unwrap();
  ...
  let drive = async move {
    return Err::<(), h3::error::ConnectionError>(std::future::poll_fn(|cx| driver.poll_close(cx)).await);
  };
