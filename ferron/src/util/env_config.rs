...
pub fn apply_env_vars_to_config(yaml_config: &mut Yaml) {
    ...
    if let Some(http2_hash) = create_or_get_hash(global_hash, "http2Settings") {
        if let Ok(val) = env::var("FERRON_HTTP2_INITIAL_WINDOW_SIZE") {
            if let Ok(size) = val.parse::<i64>() {
                http2_hash.insert(Yaml::String("initialWindowSize".into()), Yaml::Integer(size));
            }
        }
        ...
        if let Ok(val) = env::var("FERRON_HTTP2_MAX_CONCURRENT_STREAMS") {
            if let Ok(streams) = val.parse::<i64>() {
                http2_hash.insert(Yaml::String("maxConcurrentStreams".into()), Yaml::Integer(streams));
            }
        }

        if let Ok(val) = env::var("FERRON_HTTP2_MAX_HEADER_LIST_SIZE") {
            if let Ok(size) = val.parse::<i64>() {
                http2_hash.insert(Yaml::String("maxHeaderListSize".into()), Yaml::Integer(size));
            }
        }

        if let Ok(val) = env::var("FERRON_HTTP2_ENABLE_CONNECT_PROTOCOL") {
            let enable = matches!(val.to_ascii_lowercase().as_str(), "1"|"true"|"yes");
            http2_hash.insert(Yaml::String("enableConnectProtocol".into()), Yaml::Boolean(enable));
        }
    }
    ...
    if let Ok(v) = env::var("FERRON_DISABLE_NON_ENCRYPTED_SERVER") {
        let enable = matches!(v.to_ascii_lowercase().as_str(), "1"|"true"|"yes");
        global_hash.insert(Yaml::String("disableNonEncryptedServer".into()), Yaml::Boolean(enable));
    }
    
    if let Ok(v) = env::var("FERRON_ENABLE_OCSP_STAPLING") {
        let enable = matches!(v.to_ascii_lowercase().as_str(), "1"|"true"|"yes");
        global_hash.insert(Yaml::String("enableOCSPStapling".into()), Yaml::Boolean(enable));
    }

    if let Ok(v) = env::var("FERRON_ENABLE_DIRECTORY_LISTING") {
        let enable = matches!(v.to_ascii_lowercase().as_str(), "1"|"true"|"yes");
        global_hash.insert(Yaml::String("enableDirectoryListing".into()), Yaml::Boolean(enable));
    }

    if let Ok(v) = env::var("FERRON_ENABLE_COMPRESSION") {
        let enable = matches!(v.to_ascii_lowercase().as_str(), "1"|"true"|"yes");
        global_hash.insert(Yaml::String("enableCompression".into()), Yaml::Boolean(enable));
    }

    // Module loading
    if let Ok(list) = env::var("FERRON_LOAD_MODULES") {
        let arr: Vec<Yaml> = list.split(',').filter_map(|s| {
            let t = s.trim();
            if t.is_empty() { None } else { Some(Yaml::String(t.to_string())) }
        }).collect();
        if !arr.is_empty() {
    ...
    if let Ok(sni_hosts) = env::var("FERRON_SNI_HOSTS") {
        let hosts: Vec<&str> = sni_hosts.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
        
        if !hosts.is_empty() {
            let mut sni_hash = yaml_rust2::yaml::Hash::new();
            
            for host in hosts {
                let cert_env_var = format!("FERRON_SNI_{}_CERT", host.replace('.', "_").replace('*', "WILDCARD").to_uppercase());
                let key_env_var = format!("FERRON_SNI_{}_KEY", host.replace('.', "_").replace('*', "WILDCARD").to_uppercase());
    ...
    if let Ok(env_list) = env::var("FERRON_ENV_VARS") {
        let vars: Vec<&str> = env_list.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
        ...
        if !vars.is_empty() {
            ...
            if !env_hash.is_empty() {
                global_hash.insert(Yaml::String("environmentVariables".into()), Yaml::Hash(env_hash));
            }
        }
    }
}
...
fn create_or_get_hash<'a>(parent_hash: &'a mut yaml_rust2::yaml::Hash, key: &str) -> Option<&'a mut yaml_rust2::yaml::Hash> {
