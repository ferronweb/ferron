    ...
    if let Some(index_lb) = index_lb {
      ...
      if let Some(index_rb_afterlb) = index_rb_afterlb {
        ...
        match placeholder_value {
          ...
          "client_ip_canonical" => output.push_str(&socket_data.remote_addr.ip().to_canonical().to_string()),
          ...
          "server_ip_canonical" => output.push_str(&socket_data.local_addr.ip().to_canonical().to_string()),
