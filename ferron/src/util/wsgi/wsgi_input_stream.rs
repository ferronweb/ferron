...
impl WsgiInputStream {
  /// Creates a new WSGI input stream from an `AsyncBufRead`
...
impl WsgiInputStream {
  fn read(&mut self, size: usize) -> PyResult<Vec<u8>> {
    ...
    let read_bytes = futures_executor::block_on(self.body_reader.read(&mut buffer))?;
  ...
  fn readline(&mut self, size: Option<isize>) -> PyResult<Vec<u8>> {
    ...
    loop {
      let reader_buffer = futures_executor::block_on(self.body_reader.fill_buf())?.to_vec();
  ...
  fn readlines(&mut self, hint: Option<isize>) -> PyResult<Vec<Vec<u8>>> {
    ...
    loop {
      ...
      let bytes_read = futures_executor::block_on(self.body_reader.read_until(b'\n', &mut line))?;
      if bytes_read == 0 {
        break;
      }
