...
use ferron_core::{
    config::ServerConfigurationBlock, loader::ModuleLoader, registry::Registry, Module,
};
...
fn build_composite_sink(
    registry: &Registry,
    global_config: &Arc<ServerConfigurationBlock>,
) -> Result<Arc<ferron_observability::CompositeEventSink>, Box<dyn std::error::Error>> {
    ...
    Ok(Arc::new(ferron_observability::CompositeEventSink::new(
        sinks,
    )))
}
