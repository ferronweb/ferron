...
install -m 0755 data/usr/sbin/ferron-serve %{buildroot}/usr/sbin/ferron-serve

install -m 0644 data/etc/ferron/ferron.conf %{buildroot}/etc/ferron/ferron.conf
...
if [ $1 -eq 1 ]; then
    ...
    chown -R ferron:ferron /var/log/ferron /var/lib/ferron /var/www/ferron /run/ferron
    ...
    find /run/ferron -type f -exec chmod 644 {} \;
    find /run/ferron -type d -exec chmod 755 {} \;
    ...
    if type selinuxenabled >/dev/null 2>&1 && selinuxenabled; then
        ...
        if type semanage >/dev/null 2>&1; then
            ...
            semanage fcontext -a -t httpd_config_t "/etc/ferron/ferron.conf" 2>/dev/null || semanage fcontext -m -t httpd_config_t "/etc/ferron/ferron.conf" || :
            ...
            semanage fcontext -a -t httpd_var_run_t "/run/ferron(/.*)?" 2>/dev/null || semanage fcontext -m -t httpd_var_run_t "/run/ferron(/.*)?" || :
        fi

        if type restorecon >/dev/null 2>&1; then
            restorecon -r /usr/sbin/ferron{,-*} /etc/ferron/ferron.conf /var/www/ferron /var/log/ferron /var/lib/ferron || :
        fi
...
if [ $1 -eq 0 ]; then
    if type selinuxenabled >/dev/null 2>&1 && selinuxenabled; then
        ...
        if type semanage >/dev/null 2>&1; then
            ...
            semanage fcontext -d "/etc/ferron/ferron.conf" || :
            semanage fcontext -d "/var/www/ferron(/.*)?" || :
            semanage fcontext -d "/var/log/ferron(/.*)?" || :
            semanage fcontext -d "/var/lib/ferron(/.*)?" || :
            semanage fcontext -d "/run/ferron(/.*)?" || :
        fi

        if type restorecon >/dev/null 2>&1; then
            restorecon -r /usr/sbin/ferron{,-*} /etc/ferron/ferron.conf /var/www/ferron /var/log/ferron /var/lib/ferron /run/ferron || :
        fi
...
%config(noreplace) /etc/ferron/ferron.conf
