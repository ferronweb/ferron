...
cargo cyclonedx -f json --describe binaries --target "$TargetTriple" --features fips
cargo cyclonedx -f xml --describe binaries --target "$TargetTriple" --features fips
...
$FilenameNoExt = Join-Path $DistDir "ferron+fips-$FERRON_VERSION-$TargetTriple-sbom"
