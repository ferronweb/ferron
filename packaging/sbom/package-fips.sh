...
cargo cyclonedx -f json --describe binaries --target "$TARGET_TRIPLE" --features fips
cargo cyclonedx -f xml --describe binaries --target "$TARGET_TRIPLE" --features fips
...
FILENAME_NOEXT=$PREVIOUS_DIR/dist/ferron+fips-${FERRON_VERSION}-${TARGET_TRIPLE}-sbom
