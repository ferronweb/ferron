...
const NON_COMPRESSIBLE_EXTENSIONS: &[&str] = &[
    "7z", "air", "amlx", "apk", "apng", "appinstaller", "appx", "appxbundle",
    "arj", "au", "avif", "bdoc", "boz", "br", "bz", "bz2", "caf", "class",
    "doc", "docx", "dot", "dvi", "ear", "epub", "flv", "gdoc", "gif", "gsheet",
    "gslides", "gz", "iges", "igs", "jar", "jnlp", "jp2", "jpe", "jpeg", "jpf",
    "jpg", "jpg2", "jpgm", "jpm", "jpx", "kmz", "latex", "m1v", "m2a", "m2v",
    "m3a", "m4a", "mesh", "mk3d", "mks", "mkv", "mov", "mp2", "mp2a", "mp3",
    "mp4", "mp4a", "mp4v", "mpe", "mpeg", "mpg", "mpg4", "mpga", "msg", "msh",
    "msix", "msixbundle", "odg", "odp", "ods", "odt", "oga", "ogg", "ogv",
    "ogx", "opus", "p12", "pdf", "pfx", "pgp", "pkpass", "png", "pot", "pps",
    "ppt", "pptx", "qt", "ser", "silo", "sit", "snd", "spx", "stpxz", "stpz",
    "swf", "tif", "tiff", "ubj", "usdz", "vbox-extpack", "vrml", "war", "wav",
    "weba", "webm", "wmv", "wrl", "x3dbz", "x3dvz", "xla", "xlc", "xlm", "xls",
    "xlsx", "xlt", "xlw", "xpi", "xps", "zip", "zst",
];
...
fn get_paths(assets_pathbuf: &PathBuf) -> Result<Vec<PathBuf>, std::io::Error> {
    if assets_pathbuf.is_dir() {
        ...
        for entry in metadata {
            ...
            if extension.is_some_and(|ext| NON_COMPRESSIBLE_EXTENSIONS.contains(&ext)) {
                continue;
            }
...
fn compress_asset_deflate(path: &PathBuf) -> Result<(), std::io::Error> {
    ...
    let mut output = std::fs::File::create(&compressed_path)?;
    let mut encoder =
        flate2::write::DeflateEncoder::new(&mut output, flate2::Compression::default());
    std::io::copy(&mut input, &mut encoder)?;
    encoder.finish()?;
    Ok(())
}
...
fn main() {
    ...
    for assets_pathbuf in assets_pathbufs {
        paths.extend(match get_paths(&assets_pathbuf) {
            ...
            Err(err) => {
                eprintln!(
                    "Error obtaining paths at {}: {}",
                    assets_pathbuf.display(),
                    err
                );
                std::process::exit(1);
            }
